...

{
  ...
  #   when the script is used for switchback
  #
#  src_base=/busapps/ifop/acj1/fs1/EBSapps
#  dst_base=/busapps/ifop/acj1/fs1/EBSapps
#  folder=appl
#  if [ "$SWITCHBACK" = "YES" ]
#  then
#    tmp=$src_base
...
#  rm -f $outFile
 ...
 # synchOne $src_base/$folder $dst_base "--log-file=$outFile --progress -a --no-links $DELETE_FLAG" "--exclude '*.log' --exclude '*.trc'           --exclude '*.env'"  &
  ...
  #   when the script is used for switchback
  #
#  src_base=/busapps/ifop/acj1/fs2/EBSapps
 # dst_base=/busapps/ifop/acj1/fs2/EBSapps
 # folder=appl
 # if [ "$SWITCHBACK" = "YES" ]
 # then
#    tmp=$src_base
