VERSION="2.8 (Michelin PROD 09/2023)"
#
#  History
#
...
# ------------------ ------------- ------------------------------------------------------------------------------
#  2.5                             Used for Prod 1
#  2.6               09/09/2023    Using compression 
#                                  LOGS in scriptsLOG
# ------------------ ------------- ------------------------------------------------------------------------------
#
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
restore  database from service '$primDbUniqueName' section size $sectionSizeRESTORE using compressed backupset;
switch datafile all ;
switch tempfile all ;
} 
%%
# cp /tmp/rman1_$$.txt /tmp/restoreCmd.txt
    ...
    [ $? -eq 0 ] && { echo OK ; cat $LOG_TMP ; RMANLogAnalyze $LOG_TMP ; rm -f $LOG_TMP; rm -f /tmp/rman1_$$.txt ; } \
                 || { echo ERREUR ; cat $LOG_TMP ; RMANLogAnalyze $LOG_TMP  ; rm -f $LOG_TMP; die "Erreur de restauration de la base" ; rm -f /tmp/rman1_$$.txt ; }
...
%%
  ...
  [ $? -eq 0 ] && { echo OK ; cat $LOG_TMP ; RMANLogAnalyze $LOG_TMP ; rm -f $LOG_TMP; rm -f /tmp/rman2_$$.txt ; } \
               || { echo ERREUR ; cat $LOG_TMP ; RMANLogAnalyze $LOG_TMP  ; rm -f $LOG_TMP; die "Error of Recover on the database" ; rm -f /tmp/rman2_$$.txt ; }
...
sectionSizeRESTORE="32G"
# sectionSizeRECOVER="128G"
sectionSizeRECOVER="16G"
...
BASEDIR=$HOME/scriptsLOG/dataguard                      # Base dir for logs & files
...
then
  ...
  then
    ...
    [ "$dbPassword" = "" ] && dbPassword=$(getPassDB)   
